...
long int partition9(long int*,long int,long int);
int main()
{
	long int i,j,ans,k,seq[12],sum=0,lb=5,ub=50000000;
	float c;
	FILE *fptr = fopen("readings_MOM_9.csv","w");
	FILE *fp = fopen("c_MOM_9.csv","w");
	FILE *p = fopen("time_MOM_9.csv","w");
	FILE *pt = fopen("comparison_MOM_9.csv","w");
	FILE *ptr = fopen("swap_MOM_9.csv","w");
	for(i=4;i<=16;i++)
	{
	    no_of_operation=0;
	    no_of_swaps=0;
...
{
    long int i;
	long int subRight,median9,t;
    if((r - l)< 9)
    {
		return partition9(arr,l,r);
	}
    for(i=l;i<=r;i+=9)
    {
    	//no_of_operation++;
		...
		median9 = partition9(arr, i, subRight);
		//no_of_operation++;
		//no_of_swaps++;
		t=arr[median9];
		arr[median9]=arr[l + (i - l)/9];
		arr[l + (i - l)/9]=t;
	}
    return select(arr, l, l + (r - l)/9, l + (r - l)/18);
}
long int partition9(long int *arr,long int l,long int r)
{
    long int i;
	if(l>=r)
	{
