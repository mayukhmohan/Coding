...
long int partition5(long int*,long int,long int);
int main()
{
	long int i,j,ans,k,seq[12],sum=0,lb=5,ub=50000000;
	float c;
	FILE *fptr = fopen("readings_MOM.csv","w");
	FILE *fp = fopen("c_MOM.csv","w");
	FILE *p = fopen("time_MOM.csv","w");
	FILE *pt = fopen("comparison_MOM.csv","w");
	FILE *ptr = fopen("swap_MOM.csv","w");
	for(i=4;i<=16;i++)
	{
	    no_of_operation=0;
	    no_of_swaps=0;
...
long int pivot(long int *arr,long int l,long int r)
{
    ...
    for(i=l;i<=r;i+=5)
    {
		...
		//no_of_swaps++;
		t=arr[median5];
		arr[median5]=arr[l + (i - l)/5];
		arr[l + (i - l)/5]=t;
	}
    return select(arr, l, l + (r - l)/5, l + (r - l)/10);
}
long int partition5(long int *arr,long int l,long int r)
{
    long int i;
	if(l>=r)
	{
