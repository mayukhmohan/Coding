...
long int partition3(long int*,long int,long int);
int main()
{
	long int i,j,ans,k,seq[12],sum=0,lb=5,ub=50000000;
	float c;
	FILE *fptr = fopen("readings_MOM_3.csv","w");
	FILE *fp = fopen("c_MOM_3.csv","w");
	FILE *p = fopen("time_MOM_3.csv","w");
	FILE *pt = fopen("comparison_MOM_3.csv","w");
	FILE *ptr = fopen("swap_MOM_3.csv","w");
	for(i=4;i<=16;i++)
	{
	    no_of_operation=0;
	    no_of_swaps=0;
...
long int pivot(long int *arr,long int l,long int r)
{
    ...
    for(i=l;i<=r;i+=3)
    {
		...
		median3 = partition3(arr, i, subRight);
		//no_of_operation++;
		//no_of_swaps++;
		t=arr[median3];
		arr[median3]=arr[l + (i - l)/3];
		arr[l + (i - l)/3]=t;
	}
    return select(arr, l, l + (r - l)/3, l + (r - l)/6);
}
long int partition3(long int *arr,long int l,long int r)
{
    long int i;
	if(l>=r)
	{
